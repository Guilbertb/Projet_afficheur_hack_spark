...
#couleur qu'on impose pour l'écriture 
c_rouge=0x60
c_vert=0x00
c_bleu=0x01
...
screen_r =[]
for i in range(128):
    screen_r.append(range(128))

screen_v  =[]
for i in range(128):
    screen_v.append(range(128))

screen_b  =[]
for i in range(128):
    screen_b.append(range(128))
...
def affiche_chaine(x,y,chaine):

  for car in chaine:
    locate (x,y, ord(car)-(0x41-10))
    x=x+8
    
#  for rang in range(len(chaine)):
#    locate(x,y,str (chaine[rang]));
...
#creation de l'ecran en mémoire
#screen_generate()
...
affiche_chaine(10,15,"Bonjour le")
...
for i in range(128*128):
    rouge = screen_r[i / 128][i % 128]
    vert  = screen_v[i / 128][i % 128]
    bleu  = screen_b[i / 128][i % 128]
    fichier.write(chr(rouge)+chr(vert)+chr(bleu))
